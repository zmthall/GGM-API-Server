...
export const updateCommunityShown = (req, res) => {
  ...
  if (isNaN(slot) || slot < 0 || slot > 7) {
    ...
    return res.status(400).json({ message: 'Slot must be between 0 and 7' });
  }
...
export const deleteCommunityShownMedia = async (req, res) => {
  ...
  if (!slotNumber || slotNumber < 0 || slotNumber > 7) {
    return res.status(400).json({ message: 'Slot must be an integer from 0 to 7.' });
  }
