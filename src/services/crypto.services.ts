...
export function makeCryptoService(cfg: CryptoConfig): CryptoSvc {
  ...
  const decryptRideRequests = (rideDataArr: RideRequestDocument[], aad?: string): RideRequestDocument[] => {
    ...
    rideDataArr.forEach(ride => decryptedRideDataArr.push(decryptRideRequest(ride, aad)));
  ...
  const decryptContacts = (contactDataArr: ContactFormDocument[], aad?: string): ContactFormDocument[] => {
    if (!contactDataArr) return contactDataArr;

    const decryptedContactDataArr: ContactFormDocument[] = [];
    contactDataArr.forEach(contact => decryptedContactDataArr.push(decryptContact(contact, aad)));
...
const encryptApplication = (app: ApplicationData, aad?: string): ApplicationData => {
  ...
  return {
    ...
    work: {
      ...
      preferablePayRate: encrypt(toStr(app.work.preferablePayRate), aad ?? ''), // ✅ fixes number 20
...
const decryptApplications = (arr: ApplicationDocument[], aad?: string): ApplicationDocument[] => {
  if (!arr) return arr
  const out: ApplicationDocument[] = []
  arr.forEach(a => out.push(decryptApplication(a, aad)))
  return out
}
